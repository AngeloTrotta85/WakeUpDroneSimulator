...
#include "TSP2OptEnergyMinDist.h"
#include "../Loss.h"
#include "../Generic.h"

using namespace std;

TSP2OptEnergyMinDist::TSP2OptEnergyMinDist() {
	// TODO Auto-generated constructor stub

}

void TSP2OptEnergyMinDist::calculateCosts1Edge(TSP2OptEnergyEdgeMinDist *e, bool forceWakeUp, double &time, double &energy, bool log) {
	//time = 0;
	//energy = 0;
...
void TSP2OptEnergyMinDist::calculateCosts(list<TSP2OptEnergyEdgeMinDist *> edgesTSP, double &time, double &energy, bool log) {
	time = 0;
	energy = 0;
...
void TSP2OptEnergyMinDist::calculateTSP(CoordCluster *cc, std::list<Sensor *> &sl, int time_now) {
	double t, e;
	...
	for (auto& s : cc->pointsList) {
		double c = cc->clusterUAV->recharge_coord.distance(s->coord);
	...
	list<TSP2OptEnergyEdgeMinDist *> chosenCircuit;
	...
	//for (auto& as : allSensors) {
		...
		list<TSP2OptEnergyEdgeMinDist *> tmpcircuit;
		...
		if (e < cc->clusterUAV->residual_energy) {
			...
			for (auto& sss : allSensors) {
				//sss.second = Loss::getInstance().calculate_loss_full(sss.first, time_now, sl);
				sss.second = cc->clusterUAV->recharge_coord.distance(sss.first->coord);
				for (auto& chs : chosenSensors) {
					double dist = chs->coord.distance(sss.first->coord);
					if (dist < sss.second) {
						sss.second = dist;
					}
				}
...
void TSP2OptEnergyMinDist::calculateTSP_subset(Sensor *uavDummySensor , list<Sensor *> &sList, list<TSP2OptEnergyEdgeMinDist *> &fCircuit) {
	list<TSP2OptEnergyEdgeMinDist *> edges;
	list<TSP2OptEnergyEdgeMinDist *> finaledges;
	list<TSP2OptEnergyEdgeMinDist *> finalcircuit;
	...
	while (it1 != sList.end()) {
		...
		while (it2 != sList.end()) {
			double t;
			TSP2OptEnergyEdgeMinDist *ne = new TSP2OptEnergyEdgeMinDist(*it1, *it2, 0);
	...
	edges.sort(TSP2OptEnergyEdgeMinDist::sortEdges);
		...
		for (auto it_fe = finaledges.begin(); it_fe != finaledges.end(); it_fe++) {
			if (((*it_fe)->first->id == nextS->id) || ((*it_fe)->second->id == nextS->id)) {
				if ((*it_fe)->first->id == nextS->id){
					finalcircuit.push_back(new TSP2OptEnergyEdgeMinDist(nextS, (*it_fe)->second, (*it_fe)->weight));
					nextS = (*it_fe)->second;
				}
				else {
					finalcircuit.push_back(new TSP2OptEnergyEdgeMinDist(nextS, (*it_fe)->first, (*it_fe)->weight));
					nextS = (*it_fe)->first;
				}
