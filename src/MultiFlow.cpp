...
void MultiFlow::addChargStationAndUAV(MyCoord c, UAV *u) {
	...
	newcs->rechargeCounter = 0;
	newcs->sumRechargeTime = 0;
	newcs->sumRechargeTime_tslot = 0;
...
int MultiFlow::calcNumRead_Tree(void) {
	int ris = 0;

	for (auto& s : sens_list) {
		ris += s->real_readings.size();
	}

	return ris;
}

int MultiFlow::calcNumRecharge_Tot(void) {
	int ris = 0;

	for(auto& c : cs_map) {
		ris += c.second->rechargeCounter;
	}

	return ris;
}

double MultiFlow::calcNumRechargeTime_Tot(void) {
	double ris = 0;

	for(auto& c : cs_map) {
		ris += c.second->sumRechargeTime;
	}
...
void MultiFlow::calculateTreeBSF_and_UpdateMF(ChargingNode *leftmost, double end_time) {
	...
	while (true) {
		...
		double batteryRatio = min(uavEnergy / leftmost->u->max_energy, 1.0);
		double checkNum = pow(batteryRatio, Generic::getInstance().bsfExponent);

		if ( 	(uavEnergy >= leftmost->u->max_energy) ||
	...
	if (bestTSP.size() > 0) {
		if (rechargeBulk > 0) {
			...
			leftmost->sumRechargeTime_tslot += rechargeBulk * topStat.recharge_bulk;
			leftmost->sumRechargeTime += rechargeBulk * topStat.recharge_bulk * Generic::getInstance().timeSlot;
		}
...
void MultiFlow::calculateBSF(list<SensorNode *> &path, ChargingNode *cn, double tk, int tk_tslot, double uav_e,
	...
	map<pair<int,int>, double> timemap;
	list<pair<int, int>> llist;
	map<pair<int,int>, list<pair<int,int>>> pathmap;
	//map<pair<int,int>, list<pair<int,int>>> phimap;
	...
	//init
	energymap[make_pair(cn->u->id, tk_tslot)] = uav_e;
	gainmap[make_pair(cn->u->id, tk_tslot)] = 0;
	timemap[make_pair(cn->u->id, tk_tslot)] = 0;
	llist.push_back(make_pair(cn->u->id, tk_tslot));
	pathmap[make_pair(cn->u->id, tk_tslot)] = list<pair<int,int>>();
	//phimap[make_pair(cn->u->id, tk_tslot)] = list<pair<int,int>>();
	...
	while (!q.empty()) {
		...
		for (auto& sj : allSens) {
			...
			if ( (pathmap.count(sj_pair) == 0) && (residualAfter > 0) ){
				...
				gainmap[sj_pair] = energymap[qt_pair] + (1.0 - sj.second);
				timemap[sj_pair] = timemap[qt_pair] + mfTreeMatrix_time[qt_pair.first][sj_pair.first];

				llist.remove(qt_pair);
				llist.push_back(sj_pair);

				if (((double)llist.size()) > (((double)sens_map.size()) * 1.3)) {
				//if (llist.size() > sens_map.size()) {
					pair<int, int> st_pair = make_pair(-1, -1);
					double gain = numeric_limits<double>::max();
					for (auto& stprime : llist) {
						double actPathGain = gainmap[stprime];
						if (Generic::getInstance().usePsiOverTime) {
							actPathGain = actPathGain / (timemap[stprime] + mfTreeMatrix_time[stprime.first][cn->u->id]);
						}
						if ((actPathGain < gain) && (stprime != dsfPair)) {
							st_pair = stprime;
							gain = actPathGain;
						}
					}
	...
	for (auto& stprime : llist) {
		...
		double checkGain = sumgain;
		if (Generic::getInstance().usePsiOverTime) {
			checkGain = sumgain / sumtime;
		}

		if(checkGain > final_gain) {
		//if(sumGainTime > final_gain) {
			final_pair = stprime;
			final_gain = sumgain;
			//final_gain = sumGainTime;
		...
		if (centralized){
			if (s->readings.size() > 0) {
				...
				/*cout << "Calc loss for S" << sn->sens->id << " to S" << s->sens->id
						<< " -> LossD: " << Loss::getInstance().calculate_loss_distance(s->sens, sn->sens)
						<< "; LossT: " << Loss::getInstance().calculate_loss_time(r.readTime, tm)
						<< "; LossE: " << Loss::getInstance().calculate_loss_energy(sn->sens, tm_tslot, ssList)
						<< " -> LossTOT: " << actLoss
						<< endl;*/
			...
			if (uav->sensMapTree[s->sens->id].timestamp_read.size() > 0) {
				...
				/*cout << "Calc loss for S" << sn->sens->id << " to S" << s->sens->id
						<< " -> LossD: " << Loss::getInstance().calculate_loss_distance(s->sens, sn->sens)
						<< "; LossT: " << Loss::getInstance().calculate_loss_time(r, tm)
						<< "; LossE: " << Loss::getInstance().calculate_loss_energy_onNumber(uav->sensMapTree[sn->sens->id].lastResidualEnergy, tm_tslot, eeList)
						<< "; LossE(centr): " << Loss::getInstance().calculate_loss_energy(sn->sens, tm_tslot, ssList)
						<< " -> LossTOT: " << actLoss << "; penaly: " << penalty << "; final: " << Loss::algebraic_sum(actLoss, penalty)
						<< endl;*/
	...
	for (auto& s : sens_list) {
		...
		for (auto& l : s->real_wakeup) {
			if ((l.readTime > starttime) && (l.readTime <= endtime)) {
				...
				// update the energy because of the reading
				double pwu = Generic::getInstance().pwakeup;
				s->sens->residual_energy -= sensor_energy_loss_read(pwu);
...
void MultiFlow::run_tree_multiflow_distr(double end_time) {
	...
	for (auto& uav : uav_list) {
		for (auto& s : sens_list) {
			...
			uav->sensMapTree[s->sens->id].lastResidualEnergy = s->sens->residual_energy;
		}
	...
	default:
		...
		if (uav->rechargeBulk == 0) {
			if (uav->activeTSP.size() > 0) {
				...
				/*cerr << "Past readings are:" << endl;
				for (auto& s : sens_list) {
					if (uav->sensMapTree[s->sens->id].timestamp_read.size() > 0) {
						cerr << "S" << s->sens->id << " -> ";
						for (auto& re : uav->sensMapTree[s->sens->id].timestamp_read) {
							cerr << re << " ";
						}
						cerr << endl;
					}
				...
				while (true) {
					...
					double batteryRatio = min(uavEnergy / uav->cn->u->max_energy, 1.0);
					double checkNum = pow(batteryRatio, Generic::getInstance().bsfExponent);

					if ( 	(uavEnergy >= uav->cn->u->max_energy) ||
							(RandomGenerator::getInstance().getRealUniform(0, 1) <= checkNum) ) {
				...
				if (bestTSP.size() > 0) {
					...
					if (rechargeBulkSlot > 0) {
						uav->cn->rechargeCounter++;
						uav->cn->sumRechargeTime_tslot += rechargeBulkSlot * topStat.recharge_bulk;
						uav->cn->sumRechargeTime += rechargeBulkSlot * topStat.recharge_bulk * Generic::getInstance().timeSlot;
					}
